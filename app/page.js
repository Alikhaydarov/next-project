...
export default function Home(){
  ...
  async function submit(e){
    ...
    setSaving(true);
    try{await mutate({action:'payment',company_name:form.company.trim(),our_payment_method:form.method,our_card_account_text:form.method==='card'?form.ourCard.trim():null,company_card_account_text:form.method==='card'?form.clientCard.trim():null,payment_date:form.date,amount,description:form.description.trim()||null,receipt_data_url:receipt});setForm({company:'',method:form.method,ourCard:'',clientCard:'',date:localDate(),amount:'',description:''});setReceipt(null);setReceiptName('');flash('Payment saved');setTimeout(exportExcel,100)}catch(e){flash(e.message)}finally{setSaving(false)}
  }
...
function Dashboard({form,setForm,receipt,receiptName,chooseReceipt,clearReceipt,submit,saving,stats,rows,query,setQuery,setEditing,setDetails,removePayment,deletingId}){
  ...
  return <><div className="page-heading"><div><h1>Payments</h1><p>Image and description are optional. Click a payment to view them.</p></div></div><section className="stats-grid"><Stat label="Today" value={money(stats.today)}/><Stat label="This month" value={money(stats.month)}/><Stat label="All-time total" value={money(stats.total)}/><Stat label="Payments" value={stats.count}/></section><section className="workspace">
    <div className="panel"><div className="panel-title"><h2>New payment</h2><p>Card: Our Card → Client Card. Cash: card fields disappear.</p></div><form className="payment-form" onSubmit={submit}>
      ...
      <ReceiptPicker receipt={receipt} receiptName={receiptName} chooseReceipt={chooseReceipt} clearReceipt={clearReceipt}/>
...
function EditModal({item,onClose,onSave}){
  ...
  const [receiptData,setReceiptData]=useState(null),[removeReceipt,setRemoveReceipt]=useState(false),[saving,setSaving]=useState(false);
  ...
  async function submit(e){e.preventDefault();const amount=Number(form.amount.replace(/\D/g,''));const cardsOk=form.method==='cash'||(form.ourCard.trim()&&form.clientCard.trim());if(!form.company.trim()||!cardsOk||!validDate(form.date)||!amount)return;setSaving(true);try{await onSave({action:'payment_update',id:item.id,company_name:form.company.trim(),our_payment_method:form.method,our_card_account_text:form.method==='card'?form.ourCard.trim():null,company_card_account_text:form.method==='card'?form.clientCard.trim():null,payment_date:form.date,amount,description:form.description.trim()||null,receipt_data_url:receiptData,remove_receipt:removeReceipt})}finally{setSaving(false)}}
  const shownReceipt=receiptData||(!removeReceipt?item.receipt_url:null);
  return <div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><div className="edit-modal"><div className="modal-head"><div><h2>Edit payment</h2><p>Image and description are optional.</p></div><button className="modal-close" onClick={onClose}>×</button></div><form className="edit-form" onSubmit={submit}><Field label="Company name"><input value={form.company} onChange={e=>set('company',e.target.value)}/></Field><Field label="Payment method"><SelectControl><select value={form.method} onChange={e=>set('method',e.target.value)}><option value="card">Card</option><option value="cash">Cash</option></select></SelectControl></Field>{form.method==='card'&&<><Field label="Our card"><input value={form.ourCard} onChange={e=>set('ourCard',e.target.value)}/></Field><Field label="Client card"><input value={form.clientCard} onChange={e=>set('clientCard',e.target.value)}/></Field></>}<div className="form-row"><Field label="Date (YYYY-MM-DD)"><input className="date-text-input" value={form.date} maxLength={10} onChange={e=>set('date',e.target.value)}/></Field><Field label="Amount"><div className="amount-input"><span>₩</span><input value={form.amount} inputMode="numeric" onChange={e=>{const v=e.target.value.replace(/\D/g,'');set('amount',v?Number(v).toLocaleString('en-US'):'')}}/></div></Field></div><Field label="Description (optional)"><textarea className="description-input" rows={3} value={form.description} onChange={e=>set('description',e.target.value)}/></Field><div className="receipt-field"><div><strong>Receipt image</strong><span>Optional</span></div><label className="upload-button">{shownReceipt?'Replace':'Add image'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>pick(e.target.files?.[0])}/></label></div>{shownReceipt&&<div className="receipt-selected"><img src={shownReceipt} alt="Receipt"/><div><strong>Receipt attached</strong><button type="button" onClick={()=>{setReceiptData(null);setRemoveReceipt(true)}}>Remove</button></div></div>}<div className="modal-actions"><button type="button" className="cancel-btn" onClick={onClose}>Cancel</button><button className="save-edit-btn" disabled={saving}>{saving?'Saving...':'Save changes'}</button></div></form></div></div>;
}

function PaymentDetails({item,onClose,onEdit}){return <div className="modal-backdrop details-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><div className="payment-details-modal"><div className="modal-head"><div><h2>{item.company_name}</h2><p>{dateText(item)} · {methodLabel(item.our_payment_method)}</p></div><button className="modal-close" onClick={onClose}>×</button></div><div className="details-body"><div className="details-amount">{money(item.amount)}</div><div className="details-grid"><Detail label="Date" value={dateText(item)}/><Detail label="Payment method" value={methodLabel(item.our_payment_method)}/>{item.our_payment_method==='card'&&<><Detail label="Our card" value={item.our_account||'—'}/><Detail label="Client card" value={item.company_account||'—'}/></>}</div><div className="details-section"><span>Description</span><p>{item.description||'No description'}</p></div>{item.receipt_url&&<div className="details-section"><span>Receipt image</span><img className="details-receipt" src={item.receipt_url} alt="Payment receipt"/></div>}</div><div className="modal-actions details-actions"><button className="cancel-btn" onClick={onClose}>Close</button><button className="save-edit-btn" onClick={onEdit}>Edit payment</button></div></div></div>}
